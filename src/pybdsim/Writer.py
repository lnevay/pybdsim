...
class Writer():
    ...
    def WriteMachine(self, machine, filename, singlefile=False,
                     verbose=True, overwrite=True, sequence_name="lattice"):
        """
    ...
    def WriteMain(self,machine,filename=''):
        ...
        with open(fn_main, 'w') as f:
            ...
            # write this in the main file as it's often hard to customise a autogenerated model
            # if use is called and then BDSIM copies all the element definitions
            f.write(f'use, {self._sequence_name};\n')
    ...
    def WriteSequence(self, machine, filename=''):
        ...
        if self.Sequence._writeInMain:
            for line in _General.Chunks(machine.sequence, self._elementsperline):
                ...
                linetxt = '\n\t'.join(_textwrap.wrap(
                    "{}_l{}: line = ({});".format(self._sequence_name, ti, ', '.join(line))))
                self._mainFileLines.append("{}\n".format(linetxt))
                linelist.append('{}_l{}'.format(self.sequence_name, ti))
                ti += 1
            self._mainFileLines.append(f'{self._sequence_name}: line = ('+', '.join(linelist)+');\n')
            self._mainFileLines.append('\r\n')
            ...
            with open(fn_sequence, 'w') as f:
                ...
                for line in _General.Chunks(machine.sequence,self._elementsperline):
                    ...
                    linetxt = '\n\t'.join(_textwrap.wrap(
                        "{}_l{}: line = ({});".format(self._sequence_name, ti, ', '.join(line))))
                    f.write("{}\n".format(linetxt))
                    linelist.append('{}_l{}'.format(self._sequence_name, ti))
                    ti += 1

                f.write(f'{self._sequence_name}: line = ('+', '.join(linelist)+');\n')
